...
lazy_static::lazy_static! {
    ...
    pub static ref CLIENT_SERVER: ServerPtr = new_server_service();
}
...
pub fn new_server_service() -> ServerPtr {
    ...
    server.add_service(Box::new(video_service::new_video_service()));
    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    {
...
pub async fn create_tcp_connection(
    ...
    Connection::connection_start(addr, stream, id, Arc::downgrade(&server)).await;
    Ok(())
}
...
pub async fn start_server(is_server: bool) {
    ...
    if is_server {
        ...
        crate::RendezvousMediator::rendezvous_msediator_start_all().await;
    } else {
...
pub async fn start_ipc_url_server() {
    log::info!("Start an ipc server for listening to url schemes");
...
async fn sync_and_watch_config_dir() {
    ...
    log::info!("#tries of ipc service connection: {}", tries);
