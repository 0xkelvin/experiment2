...
pub fn new_video_service() -> GenericService {
...
fn create_capturer(
        ...
        #[cfg(windows)]
        {
            match scrap::CapturerMag::new(
                ...
                Ok(mut c1) => {
                    ...
                    log::info!("Create magnifier capture for {}", privacy_mode_id);
                    c = Some(Box::new(c1));
                }
                Err(e) => {
    ...
    match c {
        ...
        None => {
            log::info!("Create capturer dxgi|gdi");
            #[cfg(windows)]
...
fn run(sp: GenericService) -> ResultType<()> {
    ...
    if *SWITCH.lock().unwrap() {
        log::info!("Broadcasting display switch");
...
fn try_get_displays() -> ResultType<Vec<Display>> {
    ...
    if no_displays(&displays) {
        log::info!("no displays, create virtual display");
