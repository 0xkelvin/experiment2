...
impl Client {
    ...
    pub async fn start_new_connection(
        ...
        log::info!("start new connection: {}", peer);
...
impl AudioHandler {
    ...
    pub fn handle_frame(&mut self, frame: AudioFrame) {
        ...
        if self.simple.is_none() {
            log::info!("PulseAudio simple binding does not exists");
            return;
        }
        #[cfg(target_os = "android")]
    ...
    pub fn handle_peer_info(&mut self, pi: &PeerInfo) {
        ...
        if remember {
            if !password.is_empty() && password != password0 {
                ...
                log::info!("remember password of {}", self.id);
            }
        } else {
            if !password0.is_empty() {
                config.password = Default::default();
                log::info!("remove password of {}", self.id);
            }
        }
            ...
            if let Ok(data) = audio_receiver.recv() {
                match data {
                    ...
                    MediaData::AudioFormat(f) => {
                        log::info!("recved audio format, sample rate={}", f.sample_rate);
