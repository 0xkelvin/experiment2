...
impl<T: InvokeUiSession> Remote<T> {
    ...
    pub async fn remote_io_loop(&mut self, key: &str, token: &str) {
        ...
        match Client::start_new_connection(
            &self.handler.id,
            key,
            token,
            conn_type,
        ...
        .await
        {
            Ok((mut peer, direct, pk)) => {
                            ...
                            match _msg {
                                Some(clip) => match clip {
                                    ...
                                    _ => {
                                        ...
                                        log::info!("Process clipboard message from system, stop: {}, is_stopping_allowed: {}, server_file_transfer_enabled: {}, file_transfer_enabled: {}", stop, is_stopping_allowed, server_file_transfer_enabled, file_transfer_enabled);
                                        if stop {
                ...
                log::info!("Exit io_loop of id={}", self.handler.id);
                ...
                loop {
                    ...
                    match rx.try_recv() {
                        Ok(_) | Err(std::sync::mpsc::TryRecvError::Disconnected) => {
                            log::info!("Exit voice call audio service of client");
                            // unsubscribe
                            CLIENT_SERVER.write().unwrap().subscribe(
                                audio_service::NAME,
                                client_conn_inner,
                    ...
                    match rx_audio_data.try_recv() {
                        ...
                        Err(err) => {
                            if err == TryRecvError::Empty {
                                ...
                                log::info!("Failed to record local audio channel: {}", err);
                            }
                        }
                    }
                }
    ...
    async fn handle_msg_from_ui(&mut self, data: Data, peer: &mut Stream) -> bool {
        match data {
            ...
            Data::SendFiles((id, path, to, file_num, include_hidden, is_remote)) => {
                ...
                if is_remote {
                    log::info!("New job {}, write to {} from remote {}", id, to, path);
            ...
            match msg_in.union {
                ...
                Some(message::Union::VoiceCallRequest(request)) => {
                    if request.is_connect {
                        ...
                        log::info!("The remote has requested to close the voice call");
                ...
                Some(message::Union::VoiceCallResponse(response)) => {
                    ...
                    if let Some(ts) = ts {
                        if response.req_timestamp != ts.get() {
                            log::info!("Possible encountering a voice call attack.");
                        } else {
