...
use crate::server::{check_zombie, new_server_service as new_server, ServerPtr};
...
impl RendezvousMediator {
    ...
    pub async fn rendezvous_msediator_start_all() {
            ...
            if Config::get_option("stop-service").is_empty() {
                ...
                for host in servers.clone() {
                    ...
                    futs.push(tokio::spawn(async move {
                        allow_err!(Self::start_rendezvous_mediator(server, host).await);
    ...
    pub async fn start_rendezvous_mediator(server: ServerPtr, host: String) -> ResultType<()> {
        ...
        loop {
            let mut update_latency = || {
                ...
                if (latency - old_latency).abs() > n || old_latency <= 0 {
                    ...
                    log::info!("Latency of {}: {}ms", host, latency as f64 / 1000.);
                    old_latency = latency;
                }
            };
            select! {
                            ...
                            if let Ok(msg_in) = Message::parse_from_bytes(&bytes) {
                                match msg_in.union {
                                    Some(rendezvous_message::Union::RegisterPeerResponse(rpr)) => {
                                        update_latency();
                                        if rpr.request_pk {
                                            log::info!("rendezvous_message request_pk received from {}", host);
                                    ...
                                    Some(rendezvous_message::Union::RegisterPkResponse(rpr)) => {
                                        update_latency();
                                        log::info!("rendezvous_message register_pk_response received from {}", host);
                                    ...
                                    Some(rendezvous_message::Union::PunchHole(ph)) => {
                                        log::info!("rendezvous_message Punch hole received from {}", host);
                                        let rz = rz.clone();
                                        let server = server.clone();
                                        tokio::spawn(async move {
                                    ...
                                    Some(rendezvous_message::Union::RequestRelay(rr)) => {
                                        log::info!("rendezvous_message Request relay received from {}", host);
                                        let rz = rz.clone();
                                        let server = server.clone();
                                        tokio::spawn(async move {
                                    ...
                                    Some(rendezvous_message::Union::FetchLocalAddr(fla)) => {
                                        log::info!("rendezvous_message Fetch local addr received from {}", host);
                                        let rz = rz.clone();
                                        let server = server.clone();
                                        tokio::spawn(async move {
                                    ...
                                    Some(rendezvous_message::Union::ConfigureUpdate(cu)) => {
                                        log::info!("rendezvous_message Configure update received from {}", host);
                                ...
                                log::info!("Non-protobuf message bytes received: {:?}", bytes);
                            }
                        },
    ...
    async fn handle_intranet(&self, fla: FetchLocalAddr, server: ServerPtr) -> ResultType<()> {
        ...
        log::info!("handle_intranet from {:?}", peer_addr);
        let mut socket = socket_client::connect_tcp(&*self.host, CONNECT_TIMEOUT).await?;
    ...
    if test {
        ...
        loop {
            ...
            if query_begin.elapsed() > query_timeout {
                log::info!("query onlines timeout {:?}", query_timeout);
                break;
            }
