...
impl<T: InvokeUiSession> Session<T> {
    ...
    pub fn reconnect_xxx(&self, force_relay: bool) {
    ...
    pub async fn switch_sides(&self) {
        match crate::ipc::connect(1000, "").await {
            ...
            Err(err) => {
                log::info!("xxxserver not started (will try to start): {}", err);
            }
        }
    }
...
pub async fn io_loop<T: InvokeUiSession>(handler: Session<T>) {
    ...
    *handler.sender.write().unwrap() = Some(sender.clone());
    ...
    remote.remote_io_loop(&key, &token).await;
...
async fn start_one_port_forward<T: InvokeUiSession>(
    ...
    if let Err(err) = crate::port_forward::peer_listening(
        handler.id.clone(),
