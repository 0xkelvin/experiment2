        ...
        if let Ok(mut c) = c.try_borrow_mut() {
            match &mut *c {
                Ok(c) => match c.compress(data) {
                    Ok(res) => out = res,
                    Err(err) => {
                        crate::log::info!("Failed to compress: {}", err);
                    }
                },
                Err(err) => {
                    crate::log::info!("Failed to get compressor: {}", err);
                }
            }
        }
    });
            ...
            match &mut *d {
                Ok(d) => {
                    ...
                    match d.decompress(data, n) {
                        Ok(res) => out = res,
                        Err(err) => {
                            crate::log::info!("Failed to decompress: {}", err);
                        }
                    }
                }
                Err(err) => {
                    crate::log::info!("Failed to get decompressor: {}", err);
                }
            }
        }
    });
