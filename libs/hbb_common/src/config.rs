...
impl Config {
    fn load_<T: serde::Serialize + serde::de::DeserializeOwned + Default + std::fmt::Debug>(
        ...
        log::info!("Configuration path: {}", file.display());
    ...
    pub fn update_latency(host: &str, latency: i64) {
        ...
        if !host.is_empty() {
            ...
            if host != config.rendezvous_server {
                log::info!("Update rendezvous_server in config to {}", host);
                log::info!("{:?}", *ONLINE.lock().unwrap());
...
impl HwCodecConfig {
    ...
    pub fn refresh() {
        *HW_CODEC_CONFIG.write().unwrap() = HwCodecConfig::load();
        log::info!("HW_CODEC_CONFIG refreshed successfully");
    }
